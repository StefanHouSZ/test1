...
#------------------------------------- 此处需要修改-------------------------------------#
    $html.createHtmlReport('编目导入导出.html','媒资46系统编目导入导出相关第一轮测试','侯俊光','侯俊光')
  end
...
#通过判断页面是否有ID为spanWindowName的DIV存在来判断是否进入到了编目导入导出页面    
    if div = @ie.div(:id,'spanWindowName').exists? 
    then
#输出日志：测试成功  
    $log.logfile_pass
...
#获取该选择下拉框的已选值    
    @sl = @t1[2][4].select_list(:id,'ddlMediaType').getSelectedItems
    puts "节目类型默认选择：" 
    puts @sl    
#日志输出：需要人工再次确认一下
    $log.logfile_manual
