...
<?php
  exec("F:/Jetsen_AutoTest_project/SmartJetsen/PageLoadingTest/A-13/PageLoadTest.bat",$status);
  if ($status = 1){
  echo "SmartJetsen已经帮您完成了A-13（媒体内容管理-->其他链接（包括：音视频集合管理；检索系统（集合）；编目导入导出；节目导入导出；检索系统；视频检索系统；
					内容发布；下载审核；存储管理））的全部页面加载的性能测试！";
  }else {
